...
def get_decompressed_file_addresses_and_sizes(input, table_addr: int):
    ...
    file_addr = int.from_bytes(
        input[table_addr + pos: table_addr + pos + 4], byteorder="big"
    ) & 0xFFFFFFF
    next_file_addr = int.from_bytes(
        input[table_addr + pos + 4: table_addr + pos + 8], byteorder="big"
    ) & 0xFFFFFFF
    ...
    while file_addr != 0:
        ...
        if input[table_addr + pos] & 0xF0 == 0 or next_file_addr - file_addr > 0:
            skip_files.append(0)
            ...
            file_sizes.append(
                int.from_bytes(input[file_addr: file_addr + 4], byteorder="big") & 0xFFFFFFF
            )
...
def write_raw_files(input, buffer, table_addr):
    ...
    for i in range(0, len(file_addrs), 2):
        ...
        compressed = 0
        if skip_files[i] != 1 and file_sizes[i]:
            comp_buf = bytearray(zlib.compress(file_buf[0:file_sizes[i]], level=zlib.Z_BEST_COMPRESSION))
            padded_file = 0
            # Pad the file to 0x02
            if len(comp_buf) % 2:
                comp_buf.append(0x00)
                padded_file = 1
            file_buf = bytearray((0x80000004 + len(comp_buf) - padded_file).to_bytes(4, "big")) + comp_buf
        ...
        buffer[table_addr + (i * 4) : table_addr + (i * 4) + 4] = (compressed + pos).to_bytes(4, "big")
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Nisitenma-Ichigo zlib ROM compression Script", file=sys.stderr)
        print("", file=sys.stderr)
        print("compress.py input_file output_file", file=sys.stderr)
        print(
            "    input_file: Path to the ROM file for a decompressed Nisitenma-Ichigo zlib game.",
            file=sys.stderr,
        )
        print(
